...
void PeopVSPeop::create_server() {
  ...
  QHostAddress address = info.addresses().value(
      3);  // info.addresses() 返回一个地址表，我们取第一个
  qDebug() << info.addresses();
  qDebug() << address.toString();
  if ((itemClickCnt++ == 0) && tcpServer->listen(address, 9786)) {
    qDebug() << "PeopVSPeop resived toBuildTCP() signal and listen sucess, "
                "emit listenSuccess()";
    connect(tcpServer, SIGNAL(msgcome()), this, SLOT(readFromClient()));
    connect(this, SIGNAL(listenSuccess()), aBuildServer,
            SLOT(onListenSuccess()));
    emit listenSuccess();
  } else {
...
void PeopVSPeop::create_client() {
  ...
  if (tcpClient->ConnectedState == QAbstractSocket::ConnectedState) {
    qDebug()
        << "PeopVSPeop recieved pleaseConnectTcp() signal, connect success";
    connect(this, SIGNAL(tcpConnectSuccess()), aFindServer,
            SLOT(onTcpConnectSuccess()));
...
void PeopVSPeop::mousePressEvent(QMouseEvent *e) {
  ...
  if (pos_x >= 73 && pos_x <= 493 && pos_y >= 87 && pos_y <= 507 &&
      drawed[y][x] == 0) {
    if ((nodeStack.top == -1) ||
        (nodeStack.top >= 0 && color != nodeStack.nodes[nodeStack.top].color)) {
      RenjuNode aRenjuNode;
      aRenjuNode.x = x * DISTANCE + TO_LEFT - 13.5;
      aRenjuNode.y = y * DISTANCE + TO_UP - 14;
      ...
      if (win(chessBoard, y, x, me)) {
        ...
        QMessageBox::StandardButton reply;
        reply = QMessageBox::question(
            this, "", QString::fromUtf8("旗鼓相当，此局平局！"));
        if (reply == QMessageBox::Ok) {
          delete this;
        }
      }
...
void PeopVSPeop::exitSlot() { delete this; }
