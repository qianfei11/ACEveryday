...
void FindServer::find_server() {
  ...
  serverFinderSocket->bind(
      7758, QUdpSocket::ShareAddress);  //先建立监听，准备读server返回值
  connect(serverFinderSocket, SIGNAL(readyRead()), this,
          SLOT(readFromUdpServer()));
  QString str = "12ssYYhh" + QHostInfo::localHostName();
  QByteArray datagram = str.toUtf8();
  serverFinderSocket->writeDatagram(datagram.data(), datagram.size(),
                                    QHostAddress::Broadcast, 2089);
}
...
void FindServer::readFromUdpServer() {
  ...
  do {
    ...
    serverFinderSocket->readDatagram(datagram.data(), datagram.size(), &aQHost,
                                     &port);
  } while (serverFinderSocket->hasPendingDatagrams());

  if (datagram[0] == '1' && datagram[1] == '2' && datagram[2] == 's' &&
      datagram[3] == 's' && datagram[4] == 'Y' && datagram[5] == 'Y' &&
      datagram[6] == 'h' && datagram[7] == 'h') {
    QString hostNameStr;
