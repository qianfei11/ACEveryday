...
void BuildServer::readFromUdpClient() {
  ...
  if (datagram[0] == '1' && datagram[1] == '2' && datagram[2] == 's' &&
      ...
      datagram[6] == 'h' && datagram[7] == 'h')  //验证客户端的udp申请
  {
    QString hostNameStr;
...
void BuildServer::onListenSuccess() {
  qDebug() << "BuildServer recieved listenSuccess() signal, command "
              "PleaseConnect to FindServer";
  QByteArray datagram = "PleaseConnect";
  udpServer->writeDatagram(datagram.data(), datagram.size(), QHostAddress(IP),
                           7758);
}
